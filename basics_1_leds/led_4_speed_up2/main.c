...
void feed_forward() {
	...
	while (P2) {  // While P2 != 0
		Delay(current_delay);
		current_delay -= current_delay / 100; //  decrement current delay on 1% on each iteration
		// On last iteration current_delay will be less than 100 and thus
		// delay will decrement on zero, and thus nothing will
		// change afterwards. But at the same time, MCU is still
		// working!
